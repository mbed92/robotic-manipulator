...
void Pca9685ServoDriver::setServoUs(uint8_t channel, uint16_t target_us)
{
    ...
    if (!ensureCurrentPwmKnown(channel, target_us)) {
        return;
    }
...
bool Pca9685ServoDriver::ensureCurrentPwmKnown(uint8_t channel, uint16_t fallback_us)
{
    ...
    // The first command after this unknown state is sent directly and becomes the new reference.
    writeServoUs(channel, fallback_us);
    return false;
}
